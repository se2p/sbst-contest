...
\EVOSUITE was using more time than allocated. In one class of the FIXME project
...
mutation scores are substantially lower. The mutation scores on the FIXME
classes are highest compared to the other three projects from which classes
were taken; this is surprising since FIXME has been known for years to be
...
generation tools. While FIXME classes are affected the most, similar problems
can be observed for all projects: Considering the \budgetLong runs we found
\texttt{ClassNotFoundException}s in 414 runs for FIXME, 309 for FIXME, 308 for
FIXME, and 329 for classes in FIXME; in all these cases PIT was unable to
run even when it was possible to measure coverage.

For the first time, the competition also assessed test case readability. To this,
a human study with 13 participants was conducted. A random class under test
was selected per the \Collections, \Threeten, and \Spatial projects, for which
every competing tool generated one test case. Then, each participant was asked
to rank these test cases on a scale from 1 (best) to 5 (worst), in terms of
readability. Overall, \EVOSUITE achieved the 2nd highest rank. The rank
distribution is shown in Figure~\ref{fig:readability}.
...
where short. When \EVOSUITE ranked lowest, participants critized the lack
of comments and descriptive test names, repetitiveness in the code, and
needless assertions.

\EVOSUITE's test case for \Collections was the longest among all presented
ones. Interestingly, it ranked both highest and lowest, with participants
stating that it was ``the most complete'' and the code being ``longer making
it more confusing''.
